#在V2_train_hourly_GEOS_normal_mask192_space_date_wei_3这版模型的基础上加上当日和前一天、后一天一共三天的
#TROPOMI数据，对网络架构、输入数据进行更改
#把out_filters通道数减半
#去掉输入数据中的当日的TROPOMI数据
#给前一天和后一天的TROPOMI分别使用编码器，并给出针对性的时间戳，看看效果是否会更好
#在VT_train_4的基础上，对损失函数进行了修改，加入了GEOS强化引导和抗低估引导,并给临近时次的TROPOMI都加上了随机掩膜
#在VT_geo_train的基础上，修改了损失函数为（0.4MSEloss+0.6geoloss）；并把GEOS-CF从原始数据修改为逐月线性调整后的输入
...
from ufill.data.geo_dataset import APDataset
from ufill.models.temporal_unet import Unet
...
if __name__ == "__main__":
    ...
    lr_decay_type       = 'step'
    save_period         = 10
    save_dir            = OUTPUT_ROOT / "training" / "geo_unet"
