...
class SegFormerPredictor:
    """Load a SegFormer checkpoint and run seven-channel tensor inference."""

    def __init__(self, model_path=None, num_classes=1, cuda=True):
        if model_path is None:
            model_path = env_path("UFILL_SEGFORMER_MODEL", CHECKPOINT_ROOT / "segformer.pth")
        self.cuda = cuda and torch.cuda.is_available()
        self.net = SegFormer(num_classes=num_classes, in_channels=7)
        ...
        print(f"Loaded SegFormer checkpoint: {model_path}")
...
if __name__ == "__main__":
    ...
    # 淇濆瓨璺緞
    dir_save_path = OUTPUT_ROOT / "baselines" / "segformer"
    ...
    for annotation_line in train_lines:
        # if img_name.lower().endswith(('.nc')): 
            ...
            nc_file = Dataset(os.path.join(dir_save_path, annotation_line+'_SegFormer.nc'), 'w', format='NETCDF4')
