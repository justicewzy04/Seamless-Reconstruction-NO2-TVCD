...
import numpy as np
import os
from residual_unet_predictor import Unet
import sys
...
if __name__ == "__main__":
    ...
    # 保存路径
    dir_save_path = OUTPUT_ROOT / "baselines" / "residual_unet"
    ...
    for annotation_line in train_lines:
        # if img_name.lower().endswith(('.nc')): 
            ...
            #利用训练好的模型进行预测
            VCD_TROPOMI     = unet.detect_image(GEMS_GEOS_TROPOMI)
            ...
            nc_file = Dataset(os.path.join(dir_save_path, annotation_line+'_ResUnet.nc'), 'w', format='NETCDF4')
