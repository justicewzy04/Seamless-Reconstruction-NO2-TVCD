...
if __name__ == "__main__":
    ...
    lr_decay_type       = 'step'
    save_period         = 10
    save_dir            = OUTPUT_ROOT / "baselines" / "convlstm" / "training"
    ...
    if pretrained:
        raise ValueError(
            "Seven-channel ConvLSTM pretrained weights are not bundled. "
            "Set pretrained=False and use model_path to resume a ConvLSTM checkpoint."
        )
        
    #鍒濆鍖杕odel锛屽苟璋冪敤Unet
    model = ConvLSTM(num_classes=num_classes, in_channels=7, pretrained=pretrained, backbone=backbone).train()
    if not pretrained:
        weights_init(model)
    if model_path != '':
        if local_rank == 0:
